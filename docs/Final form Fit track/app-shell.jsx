...
// SCREEN: HOME
// ═══════════════════════════════════════════════════════════════════════════
function HomeScreen({ onNavigate, routines, sessions, loading }) {
  ...
  return (
    <div className="screen anim-fade-up">
      ...
      <div style={{ padding: '24px 20px 8px' }}>
        <div style={{ fontSize: 13, color: 'var(--text2)', fontWeight: 500, marginBottom: 4 }}>
...
// SCREEN: ROUTINES
// ═══════════════════════════════════════════════════════════════════════════
function RoutinesScreen({ routines: initialRoutines, onRoutinesChange, showToast }) {
...
// SCREEN: WORKOUT
// ═══════════════════════════════════════════════════════════════════════════
function WorkoutScreen({ routines, sessions, onSessionComplete, showToast }) {
  ...
  if (!activeRoutine) {
    return (
      <div className="screen" style={{ display: 'flex', flexDirection: 'column' }}>
        <div className="page-header"><div className="page-header-title">Entrenar</div></div>
  ...
  function renderHeatmap() {
    ...
    return (
      <div style={{ overflowX: 'auto', paddingBottom: 4 }}>
        <div style={{ display: 'grid', gridTemplateRows: 'repeat(7, 10px)', gridAutoFlow: 'column', gap: 2, width: 'fit-content' }}>
          {cells.map(c => (
            <div key={c.key} className="heatmap-cell" style={{
              background: c.count === 0 ? 'var(--surface-alt)' : `rgba(31,107,87,${0.2 + (c.count / maxCount) * 0.8})`,
            }} title={`${c.key}: ${c.count} sesión${c.count !== 1 ? 'es' : ''}`} />
          ))}
        </div>
      </div>
    );
    ...
    <div className="screen anim-fade-up">
      ...
      <div style={{ padding: '0 16px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
          <div className="section-label" style={{ padding: 0 }}>Actividad anual</div>
          <button className="btn btn-ghost btn-sm t-secondary" style={{ fontSize: 12 }} onClick={() => setHeatmapView(v => !v)}>
            {heatmapView ? 'Ocultar' : 'Ver mapa'}
          </button>
        </div>
        {heatmapView && (
          <div className="card" style={{ marginBottom: 24 }}>
            ...
            <div style={{ display: 'flex', gap: 6, alignItems: 'center', marginTop: 8 }}>
              ...
              {[0, 0.2, 0.5, 0.8, 1].map(o => (
                <div key={o} className="heatmap-cell" style={{ background: o === 0 ? 'var(--surface-alt)' : `rgba(31,107,87,${o})` }} />
              ))}
...
// SCREEN: SETTINGS
// ═══════════════════════════════════════════════════════════════════════════
function SettingsScreen({ theme, setTheme, weightUnit, setWeightUnit, showToast }) {
...
// ═══════════════════════════════════════════════════════════════════════════
function App() {
  const [appPhase, setAppPhase] = useState('launch'); // launch | onboarding | app
  ...
  return (
    <div className="ft-app" data-theme-host="">
      ...
      {appPhase === 'onboarding' && <OnboardingScreen onDone={() => setAppPhase('app')} />}

      {isInApp && (
        <>
          {activeTab === 'home' && <HomeScreen onNavigate={setActiveTab} routines={routines} sessions={sessions} loading={false} />}
          {activeTab === 'routines' && <RoutinesScreen routines={routines} onRoutinesChange={setRoutines} showToast={showToast} />}
          {activeTab === 'workout' && (
            <div className="screen-full">
              <WorkoutScreen routines={routines} sessions={sessions} onSessionComplete={handleSessionComplete} showToast={showToast} />
            </div>
          )}
          {activeTab === 'history' && <HistoryScreen sessions={sessions} />}
          {activeTab === 'stats' && <StatsScreen sessions={sessions} />}
          {activeTab === 'settings' && <SettingsScreen theme={theme} setTheme={setTheme} weightUnit={weightUnit} setWeightUnit={setWeightUnit} showToast={showToast} />}
