...
- publicada en origin
- revisadas todas las ramas locales y remotas buscando trabajo pendiente sin mergear: solo `codex/confirm-exercise-delete` (9 jun) tenia un commit sin integrar; reimplementado y la rama vieja borrada (local y remota)
- **JDK 17 instalado en la maquina de trabajo** via Homebrew (`brew install openjdk@17`, keg-only, se invoca con `JAVA_HOME=/opt/homebrew/opt/openjdk@17/libexec/openjdk.jdk/Contents/Home`). El proyecto pide JDK 17 (`sourceCompatibility`/`jvmTarget`, `app/build.gradle.kts`) y Gradle 8.7. A partir de ahora `./gradlew test`/`build` son ejecutables y verificables directamente, no solo por Codex.
- nota: `gradlew` en este repo no tiene bit de ejecucion (`-rw-r--r--`); se invoca con `sh gradlew ...` en vez de `./gradlew ...`
- **verificado con `sh gradlew test` y `sh gradlew build`: BUILD SUCCESSFUL en ambos**, cubre todo lo acumulado en `develop` (agrupacion de UI D1-D4, los 4 bugs de dogfooding, confirmacion de borrado)
...
- Pasada manual en dispositivo/emulador (test y build automaticos ya no bloquean esto, pero el flujo visual si requiere ojos humanos): borrar un ejercicio y confirmar el dialogo; los bugs B/C del selector de texto nativo (2026-07-05); el resto del flujo de entrenamiento con los ajustes de peso/reps.
